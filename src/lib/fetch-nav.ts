...
async function convertPathTreeToNavTreeNodes(
  ...
  return (
      ...
      Object.entries(docsPagePathTree).map(async ([path, obj]) => {
        ...
        if (isFolder) {
          ...
          return {
            ...
            // so as to avoid repetition, we'll default to "Overview". If
            // it's the top level file, we'll use the title of that page.
            title: !isIndex
              ? docsPage.title
              : isDocsHome
              ? docsPage.title
    ...
    // The root page gets priority, and should always come first in the list
    if (a.path == "/") {
      return -1;
    } else if (b.path == "/") {
      return 1;
