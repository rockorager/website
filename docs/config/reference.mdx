...
alic`. Disabling only `bold` or `italic` will NOT disable either
bold-italic` style. If you want to disable `bold-italic`, you must
ly disable it. You cannot partially disable `bold-italic`.
</Warning>
...
ably have to quote the keybind since `>` is a special character
shells. Example: ghostty --keybind='ctrl+a>n=new_window'
</Warning>
...
the window to be completely black. If this happens, you can
is configuration to disable the shader.
</Warning>
...
always be able to move themselves into an isolated cgroup.
</Note>
