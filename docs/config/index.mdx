...
The configuration can be reloaded at runtime by pressing `ctrl+shift+,` (Linux)
or `cmd+shift+,` (macOS). These are the default keybindings, but it is
possible to define your own via the `reload_config` action.
<Note>
  Some configuration options cannot be reloaded at runtime;
  others may only apply to newly created terminals.
</Note>
