...
(Linux) or `cmd+shift+,` (macOS).
These keybindings are configurable.
<Note>
  Not all configuration options can be reloaded at runtime;
  others may only apply to newly created surfaces or windows.
</Note>
