...
- **Secure Input**: Secure Input is the macOS system API 
that prevents accessibility APIs from reading your keystrokes, 
so things like screen recording software and so on can't read your passwords.
...
We believe that Ghostty should be a native application on each platform. Here are some key features for their respective platform.

### macOS
- **Proxy Icons**: Drag the proxy icon in the title bar to move or access terminal session files, or navigate to the file path directly.  
- **Look Up**: Tap text with three fingers to use macOS Look Up for definitions, web searches, and more.  
- **Secure Keyboard Entry**: Enable secure keyboard entry to protect passwords from being captured by other processes.  
- **Native Tabs**: Organize multiple sessions in one window with macOS’s native tab interface.  
- **Quick Terminal**: Lightweight terminal below the menu bar for instant access without interrupting work.  
